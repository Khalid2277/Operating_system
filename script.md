...
```c
Lines 50-78: Producer thread function
...
### **SECTION 5: Consumer & Remove - Ahmad (2:35-3:25) [50 seconds]**
**[Show: Lines 79-118 and Lines 135-182]**

**AHMAD:**  
"The consumer function at line 79 runs in an infinite loop starting at line 83. Line 87 calls remove_item to get data from the buffer. If it receives a poison pill—checked on line 90—it terminates gracefully. Otherwise, lines 96-99 calculate latency by comparing current time with the item's timestamp. This is our second bonus feature—performance metrics. Now here's the interesting part: the remove function at line 135 is where our first bonus feature lives—priority handling. After waiting on semaphores at lines 140-141, we scan the buffer from lines 145-156 looking for urgent items. If we find one at line 151, we extract it and shift items backward to fill the gap on lines 164-170. If no urgent items exist, we do normal FIFO removal at line 174. This ensures urgent items are always consumed first while maintaining the bounded-buffer pattern."
...
Lines 135-182: remove_item() function (with priority bonus)
  - Lines 140-141: Wait on semaphores (full, mutex)
  - Lines 145-156: Scan for urgent items (bonus feature)
  - Lines 160-171: Remove urgent and shift, or FIFO removal
  - Lines 178-179: Signal mutex and empty
```

---

### **SECTION 6: Main & Results - Khalid (3:25-3:55) [30 seconds]**
**[Show: Lines 214-216 or terminal output]**

**KHALID:**  
"In main, we initialize the buffer and semaphores on lines 214-216. Mutex is set to 1, empty to buffer_size, and full to 0—exactly as the textbook shows. We create producer threads at line 230 and consumer threads at line 239. After producers finish, we insert poison pills—one per consumer—to signal termination. Finally, lines 273-279 display performance metrics: total items, execution time, average latency, and throughput. Let me show you a quick demo."
...
Lines 249-256: Insert poison pills for graceful shutdown
Lines 273-279: Performance metrics display
```

---

### **DEMO - Ahmad (3:55-4:35) [40 seconds]**
...
"See how producers and consumers work concurrently—items marked URGENT are consumed before normal ones. After all producers finish, poison pills are inserted, and consumers exit gracefully. At the end, we get our performance metrics: all 60 items were produced and consumed, with microsecond-level latency and high throughput."

---

### **CONCLUSION - Khalid (4:35-4:55) [20 seconds]**
**[Show: Terminal with final metrics or code overview]**

**KHALID:**  
"To summarize: our implementation uses semaphores for synchronization, prevents race conditions with mutexes, implements a circular buffer with 'in' and 'out' indices, and includes both bonus features—priority handling on the consumer side and performance tracking. No deadlocks, no busy-waiting, and all items are correctly produced and consumed. Thank you for watching!"
...
| Consumer & Remove | Ahmad | 2:35-3:25 | 50s |
| Main & Setup | Khalid | 3:25-3:55 | 30s |
| Demo | Ahmad | 3:55-4:35 | 40s |
| Conclusion | Khalid | 4:35-4:55 | 20s |
| **TOTAL** | | | **4:55** |
...
| Semaphores | 25-28 | mutex, empty, full |
...
| Remove function | 135-182 | item remove_item() - with priority handling |
...
| Poison pills | 249-256 | Insert POISON_PILL items |
| Metrics display | 273-279 | printf performance metrics |
...
- Insert function (critical section logic)
...
- Consumer & Remove (complementary operations)
...
- Slow down in critical sections (insert/remove)
- Add brief pauses between speakers
- Let demo run a bit longer

**If running LONG (over 5 min):**
...
3. ✅ **Bonus 1: Priority handling** (urgent items first)
4. ✅ **Bonus 2: Performance metrics** (latency & throughput)
5. ✅ **Poison pills for graceful termination**
6. ✅ **No deadlocks, no busy-waiting**
...
- [ ] Test run produces clean output
...
### **Video Length Target:** 4:30 - 5:00 minutes (sweet spot)
...
4. **Point out line numbers** as you mention them ("As you can see on line 124...")
...
## 🔄 ALTERNATE 3-MINUTE VERSION (If Time Constraint)

**Khalid (1 min):** Intro + Semaphores + Insert  
**Ahmad (1 min):** Producer + Consumer  
**Khalid (30s):** Demo run  
**Ahmad (30s):** Conclusion  

Use this if you need a shorter version!
